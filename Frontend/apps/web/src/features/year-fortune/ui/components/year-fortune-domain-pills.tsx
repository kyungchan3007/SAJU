    ...
    >
      {domains.map((d, i) => {
        ...
        return (
          <button
            key={d.key}
            id={`year-fortune-domain-tab-${d.key}`}
            role="tab"
            type="button"
            onClick={() => onSelect(i)}
            aria-selected={isActive}
            aria-controls={`year-fortune-domain-panel-${d.key}`}
            tabIndex={isActive ? 0 : -1}
            className={`inline-flex shrink-0 cursor-pointer items-center gap-1.5 rounded-full px-5 py-[9px] text-[13px] font-bold whitespace-nowrap transition-colors transition-shadow ${
              isActive
