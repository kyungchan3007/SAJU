...
import type { InfoIconKey, InfoItem } from "../../type/types";

type Props = { items: InfoItem[] };

const INFO_ICON_MAP: Record<InfoIconKey, ReactNode> = {
  notice: (
    <svg
      fill="none"
      height="20"
...
function InfoRow({ item }: { item: InfoItem }) {
  ...
  if ("href" in item && item.href) {
    return (
      <Link
        href={item.href}
        className="group flex cursor-pointer items-center justify-between rounded-xl focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5956E9] focus-visible:ring-offset-2"
      >
        {content}
      </Link>
    );
  ...
  return (
    <button
      ...
      className="group flex w-full cursor-pointer items-center justify-between rounded-xl text-left focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-[#5956E9] focus-visible:ring-offset-2"
    >
      {content}
    </button>
  );
    ...
    >
      ...
      <div className="space-y-4">
        {items.map((item, index) => (
          <div key={item.label}>
            {index > 0 && <hr className="mb-4 border-slate-50" />}
            <InfoRow item={item} />
          </div>
        ))}
      </div>
