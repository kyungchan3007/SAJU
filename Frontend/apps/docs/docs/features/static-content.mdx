...
└── src/app/(main)/
    ...
    │   └── page.tsx
    │   └── form/
    │       └── page.tsx            ← 로그인 기반 문의 작성(Form) 페이지
    └── faq/
...
:::info 문의 작성 Form
`/contact`는 정적 MDX 안내 페이지입니다. 실제 문의 작성은 `/contact/form`에서 처리하며, 해당 페이지는 로그인 필요 페이지입니다.
Form 제출은 프론트 BFF `POST /api/inquiries`를 통해 백엔드로 전달합니다.
:::
