...
- Turnstile 검증 쿠키가 만료된 상태에서 `/api/saju/result` 호출이 `TURNSTILE_REQUIRED`를 반환하면, 사주 입력 feature는 draft를 저장한 뒤 `/verify?returnTo=/saju/result...` 로 복구시킵니다.
- 재검증이 끝나면 pending form cookie를 사용해 기존 제출 흐름을 이어갑니다.
